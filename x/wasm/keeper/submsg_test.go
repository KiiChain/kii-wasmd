...
func TestDispatchSubMsgErrorHandling(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"send tokens": {
			submsgID:         5,
			msg:              validBankSend,
			resultAssertions: []assertion{assertReturnedEvents(3), assertGasUsed(112000, 113010)},
		},
		"not enough tokens": {
			submsgID:    6,
			msg:         invalidBankSend,
		...
		"send tokens with limit": {
			...
			resultAssertions: []assertion{assertReturnedEvents(3), assertGasUsed(112000, 113150)},
		},
